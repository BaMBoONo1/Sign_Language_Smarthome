...
#Normal, Room, Living_Room, Kitchen, All, AC, Boiler, One, Two, Four, Light_On, Light_Off, On, Off, Temp_Up, Temp_Down, Enter, Check, Erase, Start
# ================= Configuration =================
GESTURE_PREFIX = "All"  # name of the gesture class
RECORD_DURATION = 2.0   # fix 1.0 seconds duration
# =================================================
        ...
        while True:
            ...
            if is_recording:
                ...
                if elapsed <= RECORD_DURATION:
                    # Retrieve the mapped joint angles and center position
                    angles = detector.get_joint_angles() # 30 angles
                    center = detector.get_hand_center() # 4 coords [rx, ry, lx, ly]
                    
                    if angles is not None and center is not None:
                        # row: [elapsed_time, rx, ry, lx, ly, angle_r0...r14, angle_l0...l14]
                        row = [round(elapsed, 4)] + [round(c, 4) for c in center] + angles
                    ...
                    if gesture_data:
                        ...
                        with open(out_path, 'w', newline='') as f:
                            writer = csv.writer(f)
                            # Write headers: time, rx, ry, lx, ly, r_angle0..14, l_angle0..14
                            headers = ["time_sec", "rx", "ry", "lx", "ly"] + \
                                      [f"r_angle_{i}" for i in range(15)] + \
                                      [f"l_angle_{i}" for i in range(15)]
                            writer.writerow(headers)
                            writer.writerows(gesture_data)
                        print(f"[{filename}] Saved {len(gesture_data)} frames of 2-hand data.")
                    else:
